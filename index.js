...
const messages = {
    start: 'Olá! Para começar, digite /iniciar.',
    help: 'Aqui estão os comandos disponíveis:\n/iniciar - Iniciar o bot\n/help - Exibir ajuda',
    welcome: (username) => `Olá, ${username}! Como posso te ajudar?`,
    buyProduct: 'Qual produto deseja comprar?',
    categorySearch: 'Em desenvolvimento...',
    zapSuiteInfo: `O Zap Suite é a solução perfeita para otimizar o atendimento no WhatsApp. Com uma interface simples e intuitiva, mesmo usuários com pouca experiência em tecnologia podem usá-lo facilmente. Grave, salve funis de vendas e automatize respostas em texto e áudio com um clique. Flexível, respeita os intervalos definidos pelo usuário. Ideal para pequenos empresários em busca de eficiência e empreendedores digitais que trabalham com WhatsApp. Versátil e acessível, o Zap Suite é a chave para um atendimento consistente e operações otimizadas.`,
    zapSuiteLink: 'Segue o link de compra: https://bit.ly/zapsuitebotautomaticodevendas',
...
bot.onText(/\/iniciar/, (msg) => {
    ...
    bot.sendMessage(chatId, messages.welcome(username), options);
});
...
bot.on('callback_query', (callbackQuery) => {
    ...
    switch (data) {
        case 'comprar_produto':
            ...
            bot.sendMessage(chatId, messages.buyProduct, comprarOptions);
            break;

        case 'pesquisar_categoria':
            bot.sendMessage(chatId, messages.categorySearch);
            break;
        ...
        case 'info_zap_suite':
            ...
            bot.sendMessage(chatId, messages.zapSuiteInfo, infoZapSuiteOptions);
            break;

        case 'link_compra_zap_suite':
            bot.sendMessage(chatId, messages.zapSuiteLink);
            break;
        ...
        case 'voltar':
            ...
            bot.sendMessage(chatId, messages.welcome(username), mainMenuOptions);
            break;

        default:
            bot.sendMessage(chatId, messages.invalidOption);
            break;
    }
});
